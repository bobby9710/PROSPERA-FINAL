  ...
  const renderGoalCard = (goal: Goal, index: number) => {
    ...
    return (
      <div
        key={goal.id}
        className={cn(
          "bg-card rounded-2xl border border-border/50 shadow-card p-4 sm:p-6 hover:shadow-lg transition-all duration-300 animate-slide-up opacity-0",
      ...
      >
        ...
        <div className="mb-3 sm:mb-4">
          <div className="flex items-end justify-between mb-2">
            <div>
              <p className="text-xl sm:text-2xl font-bold">{formatCurrency(Number(goal.current_amount))}</p>
              <p className="text-xs sm:text-sm text-muted-foreground">
                de {formatCurrency(Number(goal.target_amount))}
              </p>
            </div>
            <span className="text-base sm:text-lg font-semibold" style={{ color: goal.color }}>
        ...
        {/* Stats Grid */}
        <div className="grid grid-cols-2 gap-3 sm:gap-4 pt-3 sm:pt-4 border-t border-border/50 mb-3 sm:mb-4">
          <div className="space-y-0.5 sm:space-y-1">
            <p className="text-xs sm:text-sm text-muted-foreground">Falta</p>
            <p className="font-semibold text-sm sm:text-base">{formatCurrency(Math.max(0, remaining))}</p>
          </div>
          {daysRemaining !== null && (
            <div className="space-y-0.5 sm:space-y-1">
        ...
        {monthlyRequired !== null && !isCompleted && remaining > 0 && (
          <div className="bg-muted/50 rounded-xl p-2.5 sm:p-3 mb-3 sm:mb-4">
        ...
        {!isCompleted && (
          <div className="flex gap-2">
            ...
            {progress >= 100 && (
              <Button
                size="sm"
                className="bg-success hover:bg-success/90 text-success-foreground text-xs sm:text-sm h-9 sm:h-10"
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-6">
            <div className="stat-card p-3 sm:p-4">
              <p className="text-xs sm:text-sm text-muted-foreground mb-1">Total Economizado</p>
              <p className="text-lg sm:text-2xl font-bold">{formatCurrency(totalSaved)}</p>
            </div>
            <div className="stat-card p-3 sm:p-4">
              <p className="text-xs sm:text-sm text-muted-foreground mb-1">Falta Total</p>
              <p className="text-lg sm:text-2xl font-bold">{formatCurrency(Math.max(0, totalTarget - totalSaved))}</p>
            </div>
          </div>
